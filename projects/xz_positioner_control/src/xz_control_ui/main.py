...
class MainWindow(QMainWindow):
    ...
    def _build_ui_first_layout(self) -> None:
        ...
        if topbar_frame is not None:
            ...
            topbar_frame.show()
            topbar_root = topbar_frame

        # QMainWindow must always have a central widget placeholder.
        self.setCentralWidget(QWidget())

        self.mode_label = topbar_root.findChild(QLabel, "modeLabel") or QLabel("Sistem Modu: READY")
        ...
        self.ttl_label = topbar_root.findChild(QLabel, "ttlLabel") or QLabel("TTL Sayac: 0")
        self.gain_label = topbar_root.findChild(QLabel, "gainLabel") or QLabel("Gain (Peak): - dB")
        self.safety_label = topbar_root.findChild(QLabel, "safetyLabel") or QLabel("Safety: READY")
        self.runtime_label = topbar_root.findChild(QLabel, "runtimeLabel") or QLabel("Runtime: DEMO")
        self.runtime_mode = topbar_root.findChild(QComboBox, "runtimeModeCombo") or QComboBox()
        if self.runtime_mode.count() == 0:
            self.runtime_mode.addItems(["DEMO", "HARDWARE"])
        self.runtime_mode.currentTextChanged.connect(self._set_runtime_mode)
        self.real_vna_check = topbar_root.findChild(QCheckBox, "realVnaCheckBox") or QCheckBox("Real VNA")
        self.vna_connect_btn = topbar_root.findChild(QPushButton, "vnaConnectButton") or QPushButton("VNA Baglantisi")
        self.vna_connect_btn.clicked.connect(self._on_vna_connect_clicked)
        self.layout_btn = topbar_root.findChild(QPushButton, "layoutToggleButton") or QPushButton("Ayarlar")
        self.layout_btn.clicked.connect(self._toggle_layout_edit_mode)
        self.layout_btn.setEnabled(True)
        self.fullscreen_btn = topbar_root.findChild(QPushButton, "fullscreenButton") or QPushButton("Tam Ekran")
        self.fullscreen_btn.clicked.connect(self._toggle_fullscreen)
        self.dry_run_btn = topbar_root.findChild(QPushButton, "dryRunButton") or QPushButton("Dry-Run")
        self.dry_run_btn.clicked.connect(self._dry_run_runtime)
        topbar_layout = topbar_root.findChild(QHBoxLayout, "topbarLayout")
    ...
    def _show_info_popup(self, title: str, text: str) -> None:
        ...
        msg.setStyleSheet("QLabel { color: #7a7a7a; }")
        msg.exec()
