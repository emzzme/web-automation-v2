...
class MainWindow(QMainWindow):
    ...
    def _build_ui_first_layout(self) -> None:
        ...
        if central is None and isinstance(loaded, QMainWindow):
        ...
        self.mode_label = central.findChild(QLabel, "modeLabel") or QLabel("Sistem Modu: READY")
        self.x_label = central.findChild(QLabel, "xPosLabel") or QLabel("X Konum: 0.00 mm")
        self.z_label = central.findChild(QLabel, "zPosLabel") or QLabel("Z Konum: 0.00 mm")

        self.ttl_label = central.findChild(QLabel, "ttlLabel") or QLabel("TTL Sayac: 0")
        self.gain_label = central.findChild(QLabel, "gainLabel") or QLabel("Gain (Peak): - dB")
        self.safety_label = central.findChild(QLabel, "safetyLabel") or QLabel("Safety: READY")
        self.runtime_label = central.findChild(QLabel, "runtimeLabel") or QLabel("Runtime: DEMO")
        self.runtime_mode = central.findChild(QComboBox, "runtimeModeCombo") or QComboBox()
        if self.runtime_mode.count() == 0:
            self.runtime_mode.addItems(["DEMO", "HARDWARE"])
        self.runtime_mode.currentTextChanged.connect(self._set_runtime_mode)
        self.real_vna_check = central.findChild(QCheckBox, "realVnaCheckBox") or QCheckBox("Real VNA")
        self.vna_connect_btn = central.findChild(QPushButton, "vnaConnectButton") or QPushButton("VNA Baglantisi")
        self.vna_connect_btn.clicked.connect(self._on_vna_connect_clicked)
        self.layout_btn = central.findChild(QPushButton, "layoutToggleButton") or QPushButton("Ayarlar")
        self.layout_btn.clicked.connect(self._toggle_layout_edit_mode)
        self.layout_btn.setEnabled(True)
        self.fullscreen_btn = central.findChild(QPushButton, "fullscreenButton") or QPushButton("Tam Ekran")
        self.fullscreen_btn.clicked.connect(self._toggle_fullscreen)
        self.dry_run_btn = central.findChild(QPushButton, "dryRunButton") or QPushButton("Dry-Run")
        self.dry_run_btn.clicked.connect(self._dry_run_runtime)
        topbar_layout = central.findChild(QHBoxLayout, "topbarLayout")
        ...
        self.r_name = central.findChild(QLineEdit, "recipeNameLineEdit") or QLineEdit("Plane_XZ_Scan_5mm")
        self.r_x_start = central.findChild(QLineEdit, "xStartLineEdit") or QLineEdit("0")
        self.r_x_stop = central.findChild(QLineEdit, "xStopLineEdit") or QLineEdit("1000")
        self.r_x_step = central.findChild(QLineEdit, "xStepLineEdit") or QLineEdit("500")
        self.r_z_start = central.findChild(QLineEdit, "zStartLineEdit") or QLineEdit("0")
        self.r_z_stop = central.findChild(QLineEdit, "zStopLineEdit") or QLineEdit("1000")
        self.r_z_step = central.findChild(QLineEdit, "zStepLineEdit") or QLineEdit("500")
        self.r_snaking = central.findChild(QCheckBox, "snakingCheckBox") or QCheckBox("Snaking")
        self.apply_recipe_btn = central.findChild(QPushButton, "applyRecipeButton")
        if self.apply_recipe_btn is not None:
            self.apply_recipe_btn.clicked.connect(self._apply_recipe)

        self.x_speed = central.findChild(QLineEdit, "xSpeedLineEdit") or QLineEdit("150")
        self.z_speed = central.findChild(QLineEdit, "zSpeedLineEdit") or QLineEdit("150")
        self.x_limit_check = central.findChild(QCheckBox, "xLimitCheckBox") or QCheckBox("X Limit")
        self.z_limit_check = central.findChild(QCheckBox, "zLimitCheckBox") or QCheckBox("Z Limit")
        self.x_limit_check.stateChanged.connect(self._apply_limits)
        self.z_limit_check.stateChanged.connect(self._apply_limits)
        if (btn := central.findChild(QPushButton, "applySpeedButton")) is not None:
            btn.clicked.connect(self._apply_speed)
        if (btn := central.findChild(QPushButton, "jogXmButton")) is not None:
            btn.clicked.connect(lambda: self.system.jog("x", -50.0))
        if (btn := central.findChild(QPushButton, "jogXpButton")) is not None:
            btn.clicked.connect(lambda: self.system.jog("x", 50.0))
        if (btn := central.findChild(QPushButton, "jogZmButton")) is not None:
            btn.clicked.connect(lambda: self.system.jog("z", -50.0))
        if (btn := central.findChild(QPushButton, "jogZpButton")) is not None:
            btn.clicked.connect(lambda: self.system.jog("z", 50.0))
        if (btn := central.findChild(QPushButton, "homeButton")) is not None:
            btn.clicked.connect(self.system.home)
        if (btn := central.findChild(QPushButton, "parkButton")) is not None:
            btn.clicked.connect(self.system.park)
        if (btn := central.findChild(QPushButton, "stopButton")) is not None:
            btn.clicked.connect(self.system.stop)
        if (btn := central.findChild(QPushButton, "estopButton")) is not None:
            btn.clicked.connect(self.system.estop_press)
        if (btn := central.findChild(QPushButton, "estopResetButton")) is not None:
            btn.clicked.connect(self.system.estop_reset)

        self.vna_model_combo = central.findChild(QComboBox, "vnaModelCombo") or QComboBox()
        self.vna_start = central.findChild(QDoubleSpinBox, "vnaStartSpin") or QDoubleSpinBox()
        self.vna_stop = central.findChild(QDoubleSpinBox, "vnaStopSpin") or QDoubleSpinBox()
        self.vna_points = central.findChild(QSpinBox, "vnaPointsSpin") or QSpinBox()
        self.vna_ifbw = central.findChild(QDoubleSpinBox, "vnaIfbwSpin") or QDoubleSpinBox()
        self.vna_power = central.findChild(QDoubleSpinBox, "vnaPowerSpin") or QDoubleSpinBox()
        if (btn := central.findChild(QPushButton, "applyVnaButton")) is not None:
            btn.clicked.connect(self._apply_vna_settings)

        self.progress = central.findChild(QProgressBar, "scanProgressBar") or QProgressBar()
        self.progress_label = central.findChild(QLabel, "progressLabel") or QLabel("Ilerleme: 0/0")
        self.gain_side_label = central.findChild(QLabel, "gainSideLabel") or QLabel("Gain (Peak): - dB")
        self.report_btn = central.findChild(QPushButton, "downloadReportButton") or QPushButton("Raporu Indir")
        self.report_btn.clicked.connect(self._download_scan_report)
        self._set_report_ready(False)
        if (btn := central.findChild(QPushButton, "startScanButton")) is not None:
            btn.clicked.connect(self._start_scan)
        if (btn := central.findChild(QPushButton, "pauseScanButton")) is not None:
            btn.clicked.connect(self._pause_scan)
        if (btn := central.findChild(QPushButton, "resumeScanButton")) is not None:
            btn.clicked.connect(self._resume_scan)
        if (btn := central.findChild(QPushButton, "abortScanButton")) is not None:
            btn.clicked.connect(self._abort_scan)
        self.self_check_btn = central.findChild(QPushButton, "selfCheckButton") or QPushButton("Self-Check")
        self.self_check_btn.clicked.connect(self._run_self_check)
        self.function_test_btn = central.findChild(QPushButton, "functionTestButton") or QPushButton("Fonksiyon Testi")
        self.function_test_btn.clicked.connect(self._run_function_demo_checks)
        self.icd_load_btn = central.findChild(QPushButton, "loadIcdButton") or QPushButton("ICD Yukle")
        self.icd_load_btn.clicked.connect(self._load_icd_placeholder)
        self.icd_path_label = central.findChild(QLabel, "icdPathLabel") or QLabel("ICD: yuklenmedi")
        self.icd_map_label = central.findChild(QLabel, "icdMapLabel") or QLabel("Map: beklemede")

        self.log_table = central.findChild(QTableWidget, "logTable") or QTableWidget(0, 3)
        if (btn := central.findChild(QPushButton, "clearLogButton")) is not None:
            btn.clicked.connect(lambda: self.log_table.setRowCount(0))

        trace_host = central.findChild(QFrame, "tracePlaceholderFrame")
        ...
        heat_host = central.findChild(QFrame, "heatmapPlaceholderFrame")
        ...
        time_host = central.findChild(QFrame, "timePlaceholderFrame")
        ...
        plan_host = central.findChild(QFrame, "scanPlanPlaceholderFrame")
        ...
        def _dock_from_group(group_name: str, title: str, area: Qt.DockWidgetArea, visible: bool = True):
            grp = central.findChild(QWidget, group_name)
            if grp is None:
                return None
