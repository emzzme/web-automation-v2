...
class MainWindow(QMainWindow):
    ...
    def _build_ui_first_layout(self) -> None:
        ...
        if topbar_layout is not None:
            ...
            self.vna_connect_btn = QPushButton("VNA Baglantisi")
            self.vna_connect_btn.clicked.connect(self._on_vna_connect_clicked)
            self.layout_btn = QPushButton("Ayarlar")
            self.layout_btn.setEnabled(False)
            self.fullscreen_btn = QPushButton("Tam Ekran")
            self.fullscreen_btn.clicked.connect(self._toggle_fullscreen)
            self.dry_run_btn = QPushButton("Dry-Run")
            self.dry_run_btn.clicked.connect(self._dry_run_runtime)
        ...
        manual_layout = central.findChild(QVBoxLayout, "manualGroupLayout")
        if manual_layout is not None:
            speed_row = QHBoxLayout()
            self.x_speed = QLineEdit("150")
            self.z_speed = QLineEdit("150")
            speed_row.addWidget(QLabel("X hiz"))
            speed_row.addWidget(self.x_speed)
            speed_row.addWidget(QLabel("Z hiz"))
            ...
            for title, fn in [
                ...
                jog_row.addWidget(b)
            manual_layout.addLayout(jog_row)
            ...
            for title, fn in [
                ...
                ("Reset", self.system.estop_reset),
            ]:
                b = QPushButton(title)
                b.clicked.connect(fn)
                ctl_row.addWidget(b)
            manual_layout.addLayout(ctl_row)
            ...
            lim_row.addWidget(self.x_limit_check)
            lim_row.addWidget(self.z_limit_check)
            manual_layout.addLayout(lim_row)
        else:
            self.x_speed = QLineEdit("150")
            self.z_speed = QLineEdit("150")
            self.x_limit_check = QCheckBox("X Limit")
        ...
        right_layout = central.findChild(QVBoxLayout, "rightPanelLayout")
        if right_layout is None:
            right_layout = QVBoxLayout(self.centralWidget())
            self.centralWidget().setLayout(right_layout)

        right_layout.addWidget(self._build_scan_control_card())
        right_layout.addWidget(self._build_trace_card())
        right_layout.addWidget(self._build_heatmap_card())
        right_layout.addWidget(self._build_time_card())
        right_layout.addWidget(self._build_scan_plan_card())
        right_layout.addWidget(self._build_log_card())

        self.vna_settings_dock = QDockWidget("VNA Ayarlari", self)
        self.vna_settings_dock.setWidget(self._build_vna_settings_card())
        self.addDockWidget(Qt.DockWidgetArea.RightDockWidgetArea, self.vna_settings_dock)
